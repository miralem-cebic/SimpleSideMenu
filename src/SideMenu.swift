...
@IBDesignable public class SideMenu : UIViewController, UIGestureRecognizerDelegate {
    ...
    override public func viewDidLoad() {
        ...
        button.addTarget(self, action: #selector(SideMenu.hideMenuViewController), for: UIControlEvents.touchUpInside)
    ...
    public override func willAnimateRotation(to toInterfaceOrientation: UIInterfaceOrientation, duration: TimeInterval) {
        if (self.visible) {
            ...
            if (self.leftMenuVisible) {
                center = CGPoint(x: (UIDeviceOrientationIsLandscape(UIDevice.current.orientation) ? self.contentViewInLandscapeOffsetCenterX + self.view.frame.width : self.contentViewInPortraitOffsetCenterX + self.view.frame.width),
                                 y: self.contentViewContainer.center.y)
            } else {
                center = CGPoint(x: (UIDeviceOrientationIsLandscape(UIDevice.current.orientation) ? -self.contentViewInLandscapeOffsetCenterX : -self.contentViewInPortraitOffsetCenterX),
                                 y: self.contentViewContainer.center.y)
            }
